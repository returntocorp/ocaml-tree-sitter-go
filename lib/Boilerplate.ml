...
and map_anon_choice_type_id_ccb7464 (env : env) (x : CST.anon_choice_type_id_ccb7464) =
  (match x with
  | `Id tok -> token env tok (* identifier *)
...
and map_method_spec_list (env : env) ((v1, v2, v3) : CST.method_spec_list) =
  ...
  let v2 =
    (match v2 with
    | Some (v1, v2, v3) ->
        let v1 = map_anon_choice_type_id_ccb7464 env v1 in
        let v2 =
          List.map (fun (v1, v2) ->
            let v1 = map_anon_choice_LF_249c99f env v1 in
            let v2 = map_anon_choice_type_id_ccb7464 env v2 in
            todo env (v1, v2)
          ) v2
        in
        let v3 =
